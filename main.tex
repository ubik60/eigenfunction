...
\end{equation}
and since $\X$ is a compact space, it follows automatically from the
...
maximal $\lambda>0$. Such an eigenmeasure $\nu$ is also the Gibbs measure with respect to $\phi$ constructed above.
The existence of
...
can be obtained as the joint distribution
of the spin sequences $x\in\{-1,+1\}^{V}$ and a random graph $\gamma\in \{0,1\}^{\binom V2}$. 
The distribution of $(x,\gamma)$ is obtained by first considering the pair 
chosen independently: The spin sequence $x\in\{-1,+1\}^{V}$
according to the uniform Bernoulli measure 
$x\sim\eta(V,p=\frac12)$ on the spin sequences and the random graph $\gamma$ 
according to the Bernoulli measure $\eta(\binom V2, p)=\mscr R(V,p_J,1)$. 

The extended random cluster model $\d\mu(x,\g)$ is the joint distribution of $\g$
product measure $\d\eta(x;\frac12) \otimes \eta(x;p)$ between an independent
Bernoulli random graph $\g\sim\mscr R(V,p,1)$ random graph on $\mscr G(V)$ and 
